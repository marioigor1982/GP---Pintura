            ...
            >
              <div className="flex items-center gap-3">
                ...
                <div className="flex flex-col">
                  <span
                    className={`text-2xl md:text-3xl font-black tracking-tighter uppercase leading-none drop-shadow-lg ${
          ...
          <div className="hidden lg:flex items-center space-x-12">
            {navLinks.map((link) => (
              <a
                ...
                className={`font-bold text-base uppercase tracking-widest transition-all hover:text-orange-500 relative group/link ${
                  scrolled ? "text-slate-800" : "text-white"
                } ${activeLink === link.name ? "active-splash" : ""}`}
              >
          ...
          <div className="flex flex-col space-y-6 px-8">
            {navLinks.map((link) => (
              <a
                key={link.name}
                href={link.href}
                className={`text-slate-800 text-xl font-bold hover:text-orange-500 transition-colors relative group/link ${activeLink === link.name ? 'active-splash' : ''}`}
                onClick={() => {
